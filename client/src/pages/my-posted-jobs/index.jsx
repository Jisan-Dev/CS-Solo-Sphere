...
const MyPostedJobs = () => {
  ...
  useEffect(() => {
    axiosSecure.get(`/my-jobs?email=${user?.email}`).then((data) => setJobs(data.data));
  }, [user]);
  ...
  return (
    <section className="container px-4 mx-auto pt-12 mb-12">
      <div className="flex items-center gap-x-3">
        <h2 className="text-lg font-medium text-gray-800 ">My Posted Jobs</h2>

        <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full ">{jobs.length} Jobs</span>
      </div>
            ...
            <div className="overflow-hidden border border-gray-200  md:rounded-lg">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200 ">
                  {jobs.map((job) => (
                    <tr key={job._id}>
                      <td className="px-4 py-4 text-sm text-gray-500  whitespace-nowrap">{job?.job_title}</td>

                      <td className="px-4 py-4 text-sm text-gray-500  whitespace-nowrap">{new Date(job?.deadline).toDateString()}</td>
                      ...
                      <td title="" className="px-4 py-4 text-sm text-gray-500 ">
                        {job?.description}
                      </td>
                      <td className="px-4 py-4 text-sm whitespace-nowrap">
